...
def format_output(file_info: dict, output_format: str = "readable") -> str:
    ...
    if output_format == "json":
        ...
        return f"{file_info['file_path']},{file_info['creation_timestamp']}," + \
               f"{file_info['creation_date']},{file_info['source']},{file_info['platform']}"
...
def _print_readable_output(file_info: dict, output: str, i: int, total_files: int) -> None:
    """Print output in readable format with appropriate separators."""
    if total_files > 1 and i > 0:
        print("\n" + "="*60 + "\n")
    if total_files > 5:
...
def _show_progress(i: int, total_files: int) -> None:
    """Show progress indicator for large batches."""
    progress_pct = (i / total_files) * 100
    print(f"Progress: {i}/{total_files} ({progress_pct:.1f}%)", file=sys.stderr)


def process_multiple_files(file_paths: list, output_format: str = "readable",
    ...
    show_progress_indicator = (
        show_progress and total_files > 10 and output_format in ["readable", "timestamp"]
    )
    ...
    if show_progress_indicator:
        print(f"\nCompleted: {processed_count} files processed successfully, "
